...
ll getHash()
{
    ...
    ll res = 0;
    for (ll i = 1; i < len; i++)
        res = (res * 29 + op[i] - 'A' + 1) % (ll)1000007;
    return (ll)res;
}

VAL exist[1000010];
char hash2name[1000010][12];
ll timestamp;
int main()
{
    scanf("%lld", &n);
    for (ll i = 1; i <= n; i++)
    {
        ...
        if (op[0] == '?')
        {
            if ('0' <= op[1] && op[1] <= '9')
            {
                ...
                for (ll j = cd; j <= cd + 9; j++)
                {
                    ...
                    ll len2 = strlen(hash2name[Tree[rt].Hash] + 1);
                    for (ll i = 1; i <= len2; i++)
                        printf("%c", hash2name[Tree[rt].Hash][i]);
                    printf(" ");
                }
                printf("\n");
            }
        ...
        else
        {
            ...
            for (ll i = 1; i < len; i++)
                hash2name[Hash][i] = op[i];
            hash2name[Hash][len] = '\0';
