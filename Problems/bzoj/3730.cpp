...
void modify(int source, int x, int v)
{
    //printf("modify:source=%d x=%d v=%d\n", source, x, v);
    Tmodify(Troot[x][0], 0, n, lca(x, source), v);
    if (father[x])
        Tmodify(Troot[x][1], 0, n, lca(source, father[x]), v);
    if (x != Root)
        modify(source, father[x], v);
}

void getans(int source, int x, int v)
{
    //printf("getans:source=%d x=%d v=%d\n", source, x, v);
    ans += Tquery(Troot[x][0], 0, n, 0, v - lca(source, x));
    if (father[x])
        ans -= Tquery(Troot[x][1], 0, n, 0, v - lca(source, father[x]));
    if (x != Root)
        getans(source, father[x], v);
}
