...
#include <algorithm>
using namespace std;
const int mo = 201314, N = 11;
int n, q, head[N], tot, label, Next[N], tree[N], fa[N], dep[N], son[N], size[N],
    top[N];
int tid[N], Tree[N * 8], lazy[N * 8], ans[N][2];
bool visit[N];
struct node {
  int u, v, w, flag;
} a[N << 1];
...
void down(int id, int x) {
  if ((lazy[id] != 0) && (x > 1)) {
    Tree[id << 1] = (Tree[id << 1] + lazy[id] * (x - (x >> 1))) % mo;
    Tree[id << 1 | 1] = (Tree[id << 1 | 1] + lazy[id] * (x >> 1)) % mo;
    lazy[id << 1] += lazy[id];
    lazy[id << 1 | 1] += lazy[id];
    lazy[id] = 0;
  }
}
void up(int id) { Tree[id] = (Tree[id << 1] + Tree[id << 1 | 1]) % mo; }
...
int main() {
  ...
  for (int i = 1; i <= num; i++) {
    ...
    int t = a[i].v;
    if (!a[i].flag)
      ans[t][0] = query(a[i].w, 0);
    else
      ans[t][1] = query(a[i].w, 0);*/
  }
  for (int i = 1; i <= q; i++)
