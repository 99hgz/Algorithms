...
int vis[25], n, m, p, u, v, ff[25][25], w[25], l, iseg[1 << 22], cnt[1 << 22], s[1 << 22], invs[1 << 22];
int g[22][1 << 22], dp[22][1 << 22];
...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        ufwt(dp[i]);
        for (register int j = 0; j < l; j++)
            if(dp[i][j])dp[i][j] = (ll)dp[i][j] * invs[j] % MOD;
    }
    printf("%d\n", dp[n][l - 1]);
    system("pause");
    return 0;
