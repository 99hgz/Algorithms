...
void turnon(int source, int x)
{
    ...
    if (tmp != heap1[x].top())
    {
        ...
        if (tmp2 != heap2[father[x]].top() && lighton[father[x]])
        {
            global_heap.erase(tmp2);
            global_heap.insert(heap2[father[x]].top());
        }
...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        lighton[i] = true;
        turnon(i, i);
        global_heap.insert(heap2[i].top());
    }
    /*printf("%d\n", heap2[3].size());
    ...
    for (int i = 1; i <= Q; i++)
    {
        ...
        else
        {
            ...
            {
                turnoff(num, num);
                global_heap.erase(heap2[num].top());
                light--;
            }
            else
            {
                turnon(num, num);
                global_heap.insert(heap2[num].top());
                light++;
            }
            lighton[num] ^= 1;
        }
